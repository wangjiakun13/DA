...
def train_mpscl(model, strain_loader,sval_loader,trgtrain_loader,cfg):
    ...
    for i_iter in tqdm(range(cfg.TRAIN.MAX_ITERS+1)):
        ...
        # cla_feas_src torch.Size([16, 2048, 33, 33])
        # pred_src_aux torch.Size([16, 5, 33, 33])
        ...
        # pixel_mask torch.Size([17424])

        if cfg.TRAIN.MULTI_LEVEL:
            pred_src_aux          = interp(pred_src_aux)
            # pred_src_aux torch.Size([16, 5, 256, 256])
        ...
        pred_src_main          = interp(pred_src_main) #[16, 5, 256, 256]
        # labels_source torch.Size([16, 256, 256])
