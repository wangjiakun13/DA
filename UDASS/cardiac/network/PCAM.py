...
class VCT_Encoder(nn.Module):
    def __init__(
        ...
        for i in range(depth):
            self.cs_layers.append(nn.ModuleList([
                PreNorm(latent_dim, Attention(latent_dim, dim + 26, heads=cross_heads, dim_head=cross_dim_head),
                        context_dim=dim + 26),
    ...
    def forward(
        ...
        axis_pos = list(map(lambda size: torch.linspace(-1., 1., steps=size, device=device),
...
class VCT_Decoder(nn.Module):
    ...
    def forward(
        ...
        if self.ce_loss:
            x = x.view(b, 16, 16, 256).permute(0, 3, 1, 2)
            x = self.downsample(x)
            return x
        else:
            return x
