...
def parse_args():
    ...
    parser.add_argument('--source_domain', default='T1',
                        help='model name')
    ...
    parser.add_argument("--DA", type=str, default='T12T2')
...
def main():
    ...
    args = joblib.load('models/%s/args.pkl' %val_args.source_domain)

    if not os.path.exists('output/%s' %args.source_domain):
        os.makedirs('output/%s' %args.source_domain)
    ...
    joblib.dump(args, 'models/%s/args.pkl' %val_args.source_domain)
    ...
    # Data loading code
    img_paths = glob(r'/home1/jkwang/dataset/BraTS192D/testImage/*')
    mask_paths = glob(r'/home1/jkwang/dataset/BraTS192D/testMask/*')
    ...
    source_FE.load_state_dict(torch.load('models/%s/model.pth' %val_args.source_domain)['source_FE'])
    source_FE.eval()

    target_FE.load_state_dict(torch.load('models/%s/model.pth' %val_args.source_domain)['target_FE'])
    target_FE.eval()

    classifier.load_state_dict(torch.load('models/%s/model.pth' %val_args.source_domain)['Classifier'])
    classifier.eval()
            ...
            with torch.no_grad():
                for i, (input, target) in tqdm(enumerate(val_loader), total=len(val_loader)):
                    ...
                    if args.deepsupervision:
                        ...
                        if val_args.source_domain == 'T1':
                            feature = source_FE(t1)
                            output = classifier(feature)
                        else:
                            feature = target_FE(t1)
                            output = classifier(feature)
